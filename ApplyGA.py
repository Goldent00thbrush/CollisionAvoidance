function[Chromosomes_Childs] = ApplyGA(GA, Chromosomes, Chromosomes_Fitness) # Because number of chromosomes are not nessesarly GA.populationSize
...
if (GA.selection_option == 0) # Tournament
    ...
    [tempV] = sort(Chromosomes_Fitness, 'descend') # Sort fitness in ascending order
    ...
    WinnersIdx = V(round(rand(smallerPopulationSize, 1) * (nbrOfSelections - 1) + 1)); % Winners
    Indeces
    end

    % Crossover
    all_parents = Chromosomes(WinnersIdx,:);
    first_parents = all_parents(round(rand(smallerPopulationSize / 2, 1) * (smallerPopulationSize - 1) + 1),:); % Random
    smallerPopulationSize / 2
    Parents
    second_parents = all_parents(
        round(rand(smallerPopulationSize / 2, 1) * (smallerPopulationSize - 1) + 1),:); % Random
    smallerPopulationSize / 2
    Parents
    references_matrix = ones(smallerPopulationSize / 2, 1) * (1:GA.chromosomeLength); % The
    Reference
    Matrix
    randNums = (GA.corssoverProb_stdDev_percent * GA.chromosomeLength / 100) * randn(smallerPopulationSize / 2,
                                                                                     1) + GA.corssoverProb_mean_percent * GA.chromosomeLength / 100;
    randNums = min(round(randNums), GA.chromosomeLength); % Truncation
    randNums = max(randNums, 1); % Truncation: Vector
    of
    smallerPopulationSize / 2
    length
    of
    ...
    numbers in range
    of
    1: GA.chromosomeLength
    idx = (randNums * ones(1, GA.chromosomeLength)) > references_matrix; % Binary
    matrix
    of
    selected
    genes
    for each parents couple
